...
const FilterByTag = (props) => {
  ...
  const handleCheckboxChange = (tagName) => {
    props.setSelectedTagIds((prevSelectedTagIds) => {
      const newSelectedTagIds = new Set(prevSelectedTagIds);
      const tagId = props.tagIdMapping[tagName]; // Get the tag ID from the tag name
  ...
  return (
    <Container>
      <Row>
        {Object.entries(props.categories).map(([category, tags]) => (
